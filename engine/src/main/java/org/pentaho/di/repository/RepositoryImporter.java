  ...
  void patchTransSteps( TransMeta transMeta ) {
    for ( StepMeta stepMeta : transMeta.getSteps() ) {
      ...
      if ( stepMetaInterface instanceof HasRepositoryDirectories ) {
        patchReferencedDirectories( stepMetaInterface.isReferencedObjectEnabled(), (HasRepositoryDirectories) stepMetaInterface  );
      }
    }
  }
  ...
  private void patchJobEntries( JobMeta jobMeta ) {
    for ( JobEntryCopy copy : jobMeta.getJobCopies() ) {
      ...
      if ( jobEntryInterface instanceof HasRepositoryDirectories ) {
        patchReferencedDirectories( jobEntryInterface.isReferencedObjectEnabled(), (HasRepositoryDirectories) jobEntryInterface  );
      }
    }
  }

  private void patchReferencedDirectories( boolean[] isReferencedItem, HasRepositoryDirectories metaWithReferences ) {
    String[] referencedDirectories = metaWithReferences.getDirectories();
    if ( isReferencedItem != null && referencedDirectories != null ) {
      ObjectLocationSpecificationMethod[] specificationMethods = metaWithReferences.getSpecificationMethods();
      String[] resolvedDirectories = Arrays.copyOf( referencedDirectories, referencedDirectories.length );
      for ( int i = 0; i < isReferencedItem.length; i++ ) {
        if ( isReferencedItem[ i ] ) {
          if ( specificationMethods[ i ] == ObjectLocationSpecificationMethod.REPOSITORY_BY_NAME ) {
            if ( transDirOverride != null ) {
              ...
              resolvedDirectories[ i ] = resolvePath( baseDirectory.getPath(), referencedDirectories[ i ] );
            }
          }
        }
      }
